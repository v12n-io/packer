# ----------------------------------------------------------------------------
# Name:         centos8str.pkr.hcl
# Description:  Build definition for CentOS 8 Stream
...
source "vsphere-iso" "centos8str" {
    ...
    vm_name                     = "centos8str-${ var.build_branch }-${ local.build_version }"
    ...
    boot_command                = [ "<up><wait><tab><wait>",
                                    " text ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos8str.cfg",
                                    "<enter><wait>" ]
    ip_wait_timeout             = "20m"
    communicator                = "ssh"
...
build {
    # Build sources
    sources                 = [ "source.vsphere-iso.centos8str" ]
