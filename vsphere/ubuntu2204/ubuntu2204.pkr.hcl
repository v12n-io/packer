...
locals { 
    ...
    ds_content                  = {
                                    "meta-data" = file("${abspath(path.root)}/config/meta-data")
                                    "user-data" = templatefile("${abspath(path.root)}/config/user-data.pkrtpl.hcl", {
                                        build_username            = var.build_username
                                        build_password            = var.build_password_encrypted
...
source "vsphere-iso" "ubuntu2204" {
    ...
    shutdown_command            = "echo '${ var.build_password }' | sudo shutdown -S -E shutdown -P now"
